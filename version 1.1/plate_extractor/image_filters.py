...
def get_gaussian_blur(given_image):
    ...
    close = cv2.morphologyEx(given_image, cv2.MORPH_CLOSE, rect_kernel)
    return close
...
def apply_filter(given_image):
    given_image = get_grayscale(given_image)
    remove_small_noise(given_image)
    given_image = get_invert(given_image)
    given_image = get_gaussian_blur(given_image)
    return given_image
